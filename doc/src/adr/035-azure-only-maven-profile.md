...
- The build always passes a non-empty `-P` value; it never emits a bare `-P`. (Earlier this ADR specified "unset = no profile filter"; that is superseded, because for these profile-gated poms an unfiltered build produces no provider JAR, so a real default is both simpler and more correct.)
