...
| `pull_request_target` (base-repo context) | PR HEAD (checked out via explicit ref) | Yes | **No**: too dangerous, since it would let a PR exfiltrate the federated identity by running arbitrary code in a workflow with secret access |
