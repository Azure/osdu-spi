...
The positive-path caller is the environment's deploy identity, the same principal the lane logs in as, minted for the audience `spi info --json` publishes as `azure.token_audience`. The bearer reaches the resolver as `RESOLVER_TOKEN` and lands in every binding with `source: token`, so a suite may call the variable whatever it likes and a developer supplies the same input from `spi token`. The other two callers are the member identity, a plain user the stack seeds into `users` and each service's user group, and the no-access identity, which holds no entitlements. The stack provisions both and `spi onboard` federates them alongside the deploy identity. The lane mints their bearers for the same audience by exchanging the run's OIDC token with Entra directly, so the deploy identity's session stays in place for Restore, and they reach the resolver as `RESOLVER_MEMBER_TOKEN` and `RESOLVER_NO_ACCESS_TOKEN` for bindings with `source: memberToken` and `source: noAccessToken`; a developer supplies them from `spi token --member` and `spi token --no-access`. An environment that publishes neither identity leaves the variables unset, and only a suite that binds one reports env-not-ready.
