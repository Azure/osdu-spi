...
- **Entitlements for other services.** Partition admits any app-only token from the tenant, so the deploy identity needs no seeding there. Services that check entitlements need the stack's members Job to seed the deploy identity before their suites can join the lane, and a suite that declares `NO_ACCESS_USER` also needs the member identity from [osdu-spi-stack#208](https://github.com/Azure/osdu-spi-stack/issues/208). The no-access identity stays unseeded.
