...
**Azure/osdu-spi-stack**: ephemeral pin surface (`pin --image/--ephemeral/--run-id`, `service verify`, `reset --if-run`) · `spi onboard` + per-fork UAMI, federated credential, namespace Roles (ADR-032) · Tier-1 bootstrap: legal-tag port + test-identity ensure step · `spi load` + YAML registry + load facts · weekly reset · `loads`/`legalTag` additions to the facts envelope.
