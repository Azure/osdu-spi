...
`service.name` is the name the stack uses for the service: the `<service>` argument given to `spi onboard`, such as `partition`. The deploy lane pins, verifies, and restores under this name. It is independent of the repository name, which names the image and the GHCR package, so `Azure/osdu-spi-partition` and `danielscholl-osdu/partition` both declare `partition`. `archetype` is always `java-maven-azure`.
