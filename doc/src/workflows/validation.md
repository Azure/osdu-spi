...
- **Upstream-owned files on a PR to `main`**: Check Paths fails when a changed file exists on `origin/fork_upstream` (ADR-038), unless the PR carries the `port` label declaring a deliberate port, targets a branch other than `main`, or is a `fork_integration`/`release/upstream-*` head, mirror mode, or a fork with no `.github/upstream-filter.yml`
